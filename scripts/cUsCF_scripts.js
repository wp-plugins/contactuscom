...
//PLUGIN JQUERY ENVIROMENT (cUs_myjq)
...
cUsCF_myjq(document).ready(function($) {
    ...
    try{
        ...
        cUsCF_myjq('.advice_notice').html('Oops, something wrong happened: ' + err ).slideToggle().delay(2000).fadeOut(2000);
    }
    ...
    try{
        ...
        cUsCF_myjq('.advice_notice').html('Oops, something wrong happened: ' + err + '. Please update your WordPress version!').slideToggle().delay(2000).fadeOut(2000);
    }
    
    
    //LOGIN ALREADY CUS OR OLD CUS USERS
    cUsCF_myjq('.cUsCF_LoginUser').click(function(){
                    ...
                    success: function(data) {
                        ...
                        switch(data.status){
                            ...
                            case 3:
                                ...
                                message = '<p>Unfortunately, we weren’t able to log you into your ContactUs.com account.</p><br/>';
               ...
               cUsCF_myjq('.loadingMessage').fadeOut();
                ...
                cUsCF_myjq.ajax({ type: "POST", url: ajax_object.ajax_url, data: postData,
                    success: function(data) {
                        switch(data){
                            ...
                            default:
                                ...
                                cUsCF_myjq('.advice_notice').html(message).show().delay(10000).fadeOut(2000);
                                cUsCF_myjq('#cUsCF_CreateCustomer').val('Continue to Step 2').attr('disabled', false);
                            break;
                        }
                    ...
                    fail: function(){ //AJAX FAIL
                       message = '<p>Unfortunately there has being an error during the application. Please try again!</a></p>';
                       cUsCF_myjq('.advice_notice').html(message).show().delay(10000).fadeOut(2000);
                       cUsCF_myjq('#cUsCF_CreateCustomer').val('Continue to Step 2').attr('disabled', false); 
                    }
                });
                ...
                cUsCF_myjq.ajax({ type: "POST", url: ajax_object.ajax_url, data: {action:'cUsCF_UpdateTemplates',Template_Desktop_Form:Template_Desktop_Form,Template_Desktop_Tab:Template_Desktop_Tab},
                    success: function(data) {
                        ...
                        switch(data){
                            ...
                            case '1':
                                message = '<p>Template saved succesfuly . . . .</p>';
                                cUsCF_myjq('.notice').html(message).show().delay(1900).fadeOut(800);
                                setTimeout(function(){
                                    cUsCF_myjq('.step3').slideUp().fadeOut();
                                    cUsCF_myjq('.step4').slideDown().delay(800);
                                    location.reload();
