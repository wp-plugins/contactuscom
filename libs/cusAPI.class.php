...
class cUsComAPI_CF {
    ...
    public function getAPICredentials($cUs_email, $cUs_pass) {
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, array('X-ContactUs-Request-URL: ' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'], 'X-ContactUs-Signature: wp|4.2.5|' . $this->getIP()));
    ...
    public function createCustomer($postData, $credential) {
        ...
        if (!strlen($postData['fname'])) {
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, array(
                ...
                'X-ContactUs-Signature: wp|4.2.5|' . $this->getIP()
            ));
            $content = curl_exec($ch);
            curl_close($ch);
